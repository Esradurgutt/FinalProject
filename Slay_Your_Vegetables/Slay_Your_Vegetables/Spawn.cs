...
namespace Slay_Your_Vegetables
{
    public class SpawnManage
    {
        ...
        public void Update(GameTime gameTime, Player player)
        {
            ...
            if (spawnTimer >= lvl.CurrentLevel.spawnPeriod)//creates a new enemy when the spawnperiod expires
            {
                spawnTimer = 0f;
                int randomId = lvl.CurrentLevel.SpawnPool[random.Next(lvl.CurrentLevel.SpawnPool.Count)];//draw random enemy in swnpool
